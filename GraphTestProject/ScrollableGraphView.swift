...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    @IBInspectable open var dataPointBorderWidth: CGFloat = 2
    
    @IBInspectable open var showOnlyLastDataPoint = false
    /// The colour with which to fill the shape.
    @IBInspectable open var dataPointFillColor: UIColor = UIColor.black
    @IBInspectable open var dataPointBorderColor: UIColor?
    ...
    private var customBottomViewsData = [XCoordinateObject]()
    private var isCustomBottomViews = false
    ...
    private lazy var customBottomViewsPool: CustomBottomViewsPool = { [unowned self] in
        return CustomBottomViewsPool(self.dataPointSpacing)
        }()
    ...
    private func reset() {
        ...
        if self.isCustomBottomViews {
            self.customBottomViewsPool = CustomBottomViewsPool(self.dataPointSpacing)
        } else {
    ...
    private func addDrawingLayers(inViewport viewport: CGRect) {
        ...
        if(shouldDrawDataPoint) {
            dataPointLayer = DataPointDrawingLayer(frame: viewport, fillColor: dataPointFillColor, dataPointType: dataPointType, dataPointSize: dataPointSize, showOnlyLastDataPoint: showOnlyLastDataPoint, dataPointBorderWidth: self.dataPointBorderWidth, dataPointBorderColor: self.dataPointBorderColor, customDataPointPath: customDataPointPath)
    ...
    open func customViewSet(data: [Double], withLabelsData labelsData: [XCoordinateObject]) {
        
        // If we are setting exactly the same data and labels, there's no need to re-init everything.
        if(self.data == data && self.customBottomViewsData == labelsData) {
            return
        }
        self.isCustomBottomViews = true
        self.dataNeedsReloading = true
        self.data = data
        self.customBottomViewsData = labelsData
    ...
    private func createLinePath() -> UIBezierPath {
        ...
        if(shouldFill) {
            ...
            var viewportRightZero = CGPoint(x: lastDataPoint.x, y: zeroYPosition)
            var rightFarEdgeTop = CGPoint(x: lastDataPoint.x, y: zeroYPosition)
            var rightFarEdgeBottom = CGPoint(x: lastDataPoint.x, y: viewportHeight)
            
            if !self.showOnlyLastDataPoint {
                viewportRightZero = CGPoint(x: lastDataPoint.x + (rightmostPointPadding), y: zeroYPosition)
                rightFarEdgeTop = CGPoint(x: lastDataPoint.x + (rightmostPointPadding + viewportWidth), y: zeroYPosition)
                rightFarEdgeBottom = CGPoint(x: lastDataPoint.x + (rightmostPointPadding + viewportWidth), y: viewportHeight)
            }
    ...
    private func activePointsDidChange() {
        ...
        if(shouldShowLabels) {
            ...
            if self.isCustomBottomViews {
                self.updateCustomBottomViews(deactivatedPoints: deactivatedPoints, activatedPoints: activatedLabelPoints)
            } else {
                self.updateLabels(deactivatedPoints: deactivatedLabelPoints, activatedPoints: activatedLabelPoints)
            }
        }
    }
    ...
    private func updateLabels(deactivatedPoints: [Int], activatedPoints: [Int]) {
        ...
        for point in activatedPoints {
            ...
            //let customView = self.createCustomLabelView(withObject: labels[point])
            
            //customView.frame = CGRect(origin: CGPoint(x: position.x - customView.frame.width / 2, y: position.y + dataPointLabelTopMargin), size: customView.frame.size)
    ...
    // MARK: Custom IC LabelsX
    private func updateCustomBottomViews(deactivatedPoints: [Int], activatedPoints: [Int]) {
        
        // Disable any labels for the deactivated points.
        for point in deactivatedPoints {
            customBottomViewsPool.activateView(forPointIndex: point)
        }
        
        // Grab an unused label and update it to the right position for the newly activated poitns
        for point in activatedPoints {
            let view = self.customBottomViewsPool.activateView(forPointIndex: point)
            
            let obj: XCoordinateObject = self.customBottomViewsData[point]
            
            view.upperTextLabel.text = obj.name
            view.upperTextLabel.textColor = dataPointLabelColor
            view.upperTextLabel.font = dataPointLabelFont
            
            view.bottomTextLabel.text = obj.value
            view.bottomTextLabel.textColor = dataPointLabelColor
            view.bottomTextLabel.font = dataPointLabelFont
            
            let rangeMin = (shouldAutomaticallyDetectRange || shouldAdaptRange) ? self.range.min : self.rangeMin
            let position = calculatePosition(atIndex: point, value: rangeMin)
            
            view.frame = CGRect(origin: CGPoint(x: position.x - view.frame.width / 2,
                                                y: position.y + dataPointLabelTopMargin),
                                size: view.frame.size)
            
            let _ = labelsView.subviews.filter { $0.frame == view.frame }.map { $0.removeFromSuperview() }
    ...
    private func repositionActiveLabels() {
        ...
        if self.isCustomBottomViews {
            for view in self.customBottomViewsPool.activeViews {
                let rangeMin = (shouldAutomaticallyDetectRange || shouldAdaptRange) ? self.range.min : self.rangeMin
                let position = calculatePosition(atIndex: 0, value: rangeMin)
                
                view.frame.origin.y = position.y + dataPointLabelTopMargin
            }
        } else {
...
// MARK: - CustomBottomViewsPool <------
private class CustomBottomViewsPool {
    
    var views = [CustomBottomView]()
    var relations = [Int : Int]()
    var unused = [Int]()
    var dataPointSpacing: CGFloat!
    
    init(_ dataPointSpacing: CGFloat) {
        self.dataPointSpacing = dataPointSpacing
    }
    
    func deactivateView(forPointIndex pointIndex: Int){
    ...
    @discardableResult
    func activateView(forPointIndex pointIndex: Int) -> CustomBottomView {
        var view: CustomBottomView
        
        if(unused.count >= 1) {
            let unusedLabelIndex = unused.first!
            unused.removeFirst()
            
            view = views[unusedLabelIndex]
            relations[pointIndex] = unusedLabelIndex
        } else {
            view = CustomBottomView(dataPointSpacing: self.dataPointSpacing)
            
            views.append(view)
            let newLabelIndex = views.index(of: view)!
            relations[pointIndex] = newLabelIndex
        }
        return view
    }
    
    var activeViews: [CustomBottomView] {
        get {
            var currentlyActive = [CustomBottomView]()
            let numberOfLabels = views.count
            
            for i in 0 ..< numberOfLabels {
                if(!unused.contains(i)) {
                    currentlyActive.append(views[i])
                }
            }
            return currentlyActive
        }
...
private class DataPointDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    private var showOnlyLastDataPoint = false
    
    private var customDataPointPath: ((_ centre: CGPoint) -> UIBezierPath)?
    
    init(frame: CGRect, fillColor: UIColor, dataPointType: ScrollableGraphViewDataPointType, dataPointSize: CGFloat, showOnlyLastDataPoint: Bool, dataPointBorderWidth: CGFloat?, dataPointBorderColor: UIColor?, customDataPointPath: ((_ centre: CGPoint) -> UIBezierPath)? = nil) {
        
        self.dataPointType = dataPointType
        self.dataPointSize = dataPointSize
        self.customDataPointPath = customDataPointPath
        self.showOnlyLastDataPoint = showOnlyLastDataPoint
        super.init(viewportWidth: frame.size.width, viewportHeight: frame.size.height)
        //dataPointBorderWidth
    ...
    private func createDataPointPath() -> UIBezierPath {
        ...
        func appenPoint(atIndex index: Int) {
            var location = CGPoint.zero
            
            if let pointLocation = self.graphViewDrawingDelegate?.graphPoint(forIndex: index).location {
                location = pointLocation
            }
                print("Point: \(index) - \(location)")
        ...
        if self.showOnlyLastDataPoint {
            appenPoint(atIndex: activePointsInterval.last!)
        } else {
            for i in activePointsInterval {
                appenPoint(atIndex: i)
            }
        }
